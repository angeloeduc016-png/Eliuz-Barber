window.EliuzStorage = (() => {
  ...
  async function saveBooking(booking) {
    ...
    return request(
      ...
      () => {
        ...
        const alreadyBooked = bookings.some(item => (item.data || item.date) === booking.date && (item.horario || item.time) === booking.time);
        if (alreadyBooked) {
        ...
        const normalized = {
          ...
          valor: Number(booking.value) || SERVICE_CATALOG[booking.service]?.price || 0,
          data: booking.date,
        ...
        write(BOOKING_KEY, bookings);
        return { ok: true, booking: normalized, notificationSent: false };
      }
    ).then((payload) => ({
      ok: true,
  ...
  async function deleteBooking(id, options = {}) {
    ...
    return request(
      ...
      () => {
        const bookings = getLocalBookings().filter(item => item.id !== id);
        write(BOOKING_KEY, bookings);
        return { ok: true };
      }
    );
  ...
  async function saveCashEntry(entry, options = {}) {
    ...
    return request(
      ...
      () => {
        const entries = getLocalCashEntries();
        const normalized = {
          id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
          type: entry.type,
          description: entry.description,
  ...
  async function deleteCashEntry(id, options = {}) {
    ...
    return request(
      ...
      () => {
        const entries = getLocalCashEntries().filter(item => item.id !== id);
        write(CASH_KEY, entries);
        return { ok: true };
      }
    );
